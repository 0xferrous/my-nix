...
| `codex-desktop` | ChatGPT/Codex Linux desktop app (preview): closed-source Electron bundle of the Apache-2.0 `codex` CLI; ships as `.deb` from OpenAI's `latest/` URL, unpacked and patched here (unfree, `--no-sandbox`) |
