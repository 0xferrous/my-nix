# Generated zig package cache for the libghostty-vt native build.
#
# Generated from ghostty build.zig.zon(.json) at commit
# a887df42c56f6de86c0fe6da9c4eeca37931e083 (the commit pinned by
# libghostty-vt-sys 0.2.1). Every URL dependency from the zon file is
# prefetched because ghostty's build graph resolves its lazy dependencies
# eagerly. Tarball hashes are the sha256 of the downloaded artifact bytes;
# the link-farm entry names are the zig package hashes from the zon.
{
  lib,
  fetchurl,
  fetchgit,
...
let
  ...
  fetchZigDep =
    ...
    if lib.hasPrefix "git+" url then
      fetchGitZig {
        inherit name hash;
        url = lib.removePrefix "git+" url;
      }
    else
      fetchZigArtifact { inherit name url hash; };

  # Zig's build runner computes relative paths from `cwd` to the build
  # directory lexically, so a symlinked cache dir (linkFarm) can resolve to a
  # different depth than the real path. Copy instead of symlinking.
