...
- keep the README's "Overlay & packages" section in sync when adding/renaming/removing packages in `pkgs/` or `pkgs/overlay.nix`, and mirror new overlay packages into `packages.<system>.*` in the root `flake.nix` when they are generally useful
- only document packages in the README that are genuinely reusable by others; keep personal/niche packages (e.g. `piDev`, `nash`) out of the README table or note their caveats if included
